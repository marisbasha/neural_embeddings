...
class WhisperSegmenterFast(SegmenterBase):
    def __init__(self, model_path, device=None, device_ids = [ 0,] ):
        ...
        if os.path.isfile(tokenizer_file):
            self.tokenizer_list = [tokenizers.Tokenizer.from_file(tokenizer_file) for _ in self.device_list ]
        else:
            self.tokenizer_list = [tokenizers.Tokenizer.from_pretrained(
                "openai/whisper-tiny" + ("" if self.model.is_multilingual else ".en")
            ) for _ in self.device_list ]
        # self.tokenizer_list = [ WhisperTokenizer.from_pretrained(model_path ) for _ in self.device_list ]
        
        model_config = json.load(open(model_path+"/config.json"))        
        self.total_spec_columns = model_config["total_spec_columns"] if "total_spec_columns" in model_config else 1000 
        self.cluster_codebook = {"0": 0}
        self.inverse_cluster_codebook = { cluster_id:cluster  for cluster, cluster_id in self.cluster_codebook.items() }
